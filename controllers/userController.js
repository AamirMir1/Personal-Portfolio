...
exports.createUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (newUser) {
    ...
    const verificationUrl = `${req.protocol}://${req.get(
      "host"
    )}/verify/email/${getVerificationToken}`;
...
exports.accountVerification = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler(
        "Verification Failed: Token is expired or has invalid",
        401
      )
    );
  }
...
exports.deleteProject = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!isProjectExists) {
    return next(
      new ErrorHandler(`Project is not found with this id: ${id}`, 404)
    );
  }
...
exports.updatePersonalDetails = catchAsyncErrors(async (req, res, next) => {
  ...
  if (email !== user.email) {
    ...
    const updateEmailUrl = `${req.protocol}://${req.get(
      "host"
    )}/update/${updateEmailToken}?email=${email}`;
...
exports.updateEmailRequest = catchAsyncErrors(async (req, res, next) => {
  ...
  const user = await User.findOne({
    updateEmailToken,
    updateEmailTokenExpires: { $gt: Date.now() },
  });
...
exports.resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!req.body.newPassword || !req.body.confirmPassword) {
    return next(
      new ErrorHandler("Please enter new password and confirm password", 400)
    );
  }

  if (req.body.newPassword !== req.body.confirmPassword) {
    return next(
      new ErrorHandler("New password and confirm password is not matching", 400)
    );
  }

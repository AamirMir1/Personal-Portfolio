...
export const loginUser = ({ email, password }) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("http://localhost:5000/login", userData, config)
...
export const forgetPassword = ({ forgetEmail }) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("http://localhost:5000/forget", { email: forgetEmail }, config)
...
export const resetPassword = ({ newPassword, confirmPassword, token }) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:5000/reset/${token}`, userData, config)
...
export const sendContact = ({ userName, userEmail, userMessage }) => async (dispatch) => {
    try {
        ...
        await axios.post("http://localhost:5000/contact", userData, config)
...
export const deleteSkill = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:5000/skill/delete/${id}`)
...
export const deleteProject = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:5000/project/delete/${id}`)
